...
for(i in 1:R){
    ...
    prob3_fit = predict(fit3.boot, type='response', data.boot)
    pred3_fit = prediction(prob3_fit, data.boot$correct)
    auc3_fit = performance(pred3_fit,"auc")@y.values[[1]][1]
    B[i, 2] = auc3_fit
    
    B[i,3] = auc3_fit-auc3_org
    ...
    C[i, 3] = 1 - C[i, 1]/(mean(predict_boot)*(1 - mean(predict_boot)))
